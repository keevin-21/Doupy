...
def minigame_dragons(): 
    ...
    pygame.display.set_caption("Con huevos Link!")
    ...
    def draw():
        nonlocal lairs, eggs_collected, lives, game_complete
    ...
    def handle_movement(keys):
        nonlocal index_derecha, index_izquierda, index_abajo, index_arriba
    ...
    def check_for_collisions():
        nonlocal lairs, eggs_collected, lives, reset_required, game_complete
        for lair in lairs:
            if not lair["egg_hidden"]:
    ...
    def check_for_egg_collision(lair):
        nonlocal eggs_collected, game_complete
    ...
    def substract_life():
        nonlocal lives, game_over, reset_required
        lives -= 1
        if lives <= 0:
            game_over = True
        else:
